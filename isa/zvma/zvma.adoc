= Zvma Family of Vector Matrix Extensions
...
Attempts to execute any instruction that accesses the tile state or the
`mtype` CSR raise an illegal-instruction exception when `mstatus`.MS is
set to Off.

When `mstatus`.MS is set to Initial or Clean, executing any instruction that
changes tile state or the `mtype` CSR will change `mstatus`.MS to Dirty.
Implementations may also change `mstatus`.MS from Initial or Clean to Dirty
at any time, even when there is no change in tile state or `mtype`.
...
execute any instruction that accesses tile state or the `mtype` CSR raise an
illegal-instruction exception when either field is set to Off.

NOTE: The `vsettn` instruction is not affected by the setting of the MS
field, but the `vsettm` and `vsettk` instructions are.

When V=1 and neither `vsstatus`.MS nor `mstatus`.MS is set to Off, executing
any instruction that changes tile state or the `mtype` CSR will change both
`mstatus`.MS and `vsstatus`.MS to Dirty.
Implementations may also change `mstatus`.MS or `vsstatus`.MS from Initial or
Clean to Dirty at any time, even when there is no change in tile state or
`mtype`.
...
The `vtdiscard` instruction is provided to inform the runtime that the tile
state and the contents of the `mtype` CSR are no longer useful and need not be
saved.
`vtdiscard` raises an illegal-instruction exception if `mstatus`.MS=Off (or if
V=1 and `vsstatus`.MS=Off).
Otherwise, it changes `mstatus`.MS to Initial
...
NOTE: `vtdiscard` does not actually write the tile state or `mtype`.
For ABI purposes, the tile state and `mtype` becomes UNPREDICTABLE as a result
of executing this instruction.
