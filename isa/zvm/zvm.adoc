...
When `altfmt`=1 and SEW=16, all matrix multiply instructions become reserved,
except for the following, which are redefined to use the BF16 format for any
operand that would otherwise have used the FP16 format:

- `vfmm.tvv` (when the Zvma32a16f extension is implemented)
...
`vlte` and `vste` support TEW values corresponding to all
supported SEW settings (8..ELEN).
...
vlte8.t  rs2, (rs1) // rs1 is memory address, rs2 is TSS
vlte16.t rs2, (rs1) // rs1 is memory address, rs2 is TSS
vlte32.t rs2, (rs1) // rs1 is memory address, rs2 is TSS
vlte64.t rs2, (rs1) // rs1 is memory address, rs2 is TSS

// Store tile subset to memory, using TEW=EEW
vste8.t  rs2, (rs1) // rs1 is memory address, rs2 is TSS
vste16.t rs2, (rs1) // rs1 is memory address, rs2 is TSS
vste32.t rs2, (rs1) // rs1 is memory address, rs2 is TSS
vste64.t rs2, (rs1) // rs1 is memory address, rs2 is TSS
...
#   8      4       0    E4M3  E4M3  FP32 (Zvm32a8f)
#   8      4       1    E4M3  E5M2  FP32 (Zvm32a8f)
#  16      2       0    FP16  FP16  FP32 (Zvm32a16f)
#  32      1       0    FP32  FP32  FP32 (Zvm32a32f)
#  64      1       0    FP64  FP64  FP64 (Zvm64a64f)
vfmm.tvv     mtd, vs2, vs1

# SEW  TWIDEN  altfmt    vs2   vs1   mtd
#   8      4       0    E5M2  E4M3  FP32 (Zvm32a8f)
#   8      4       1    E5M2  E5M2  FP32 (Zvm32a8f)
#  16      2       1    BF16  BF16  FP32 (Zvm32a16f)
vfmm.alt.tvv mtd, vs2, vs1


# Integer Matmuls

# SEW  TWIDEN  altfmt    vs2   vs1   mtd
#   8      4       0   uint8 uint8 int32 (Zvm32a8i)
#   8      4       1   uint8  int8 int32 (Zvm32a8i)
vmmu.tvv mtd, vs2, vs1

# SEW  TWIDEN  altfmt   vs2   vs1    mtd
#   8      4       0   int8 uint8  int32 (Zvm32a8i)
#   8      4       1   int8  int8  int32 (Zvm32a8i)
vmms.tvv mtd, vs2, vs1
----
...
multiply instructions with SEW=8 and TWIDEN=4.

----
vfp2mm.tvv mtd, vs2, vs1   # OCP MX FP4, accumulate into FP32 (Zvm32a4f)
----

Each 8-bit input element is divided into two 4-bit fields.  The
`vfp2mm.tvv` instruction treats `tm`, `tn`, and `tk` values as defining the
...
31   26 25 24  20 19  15 14 12  1110 9 8 7 6     0
 ...
 111100  1   vs2    vs1    001   t t t 0 0  1110111 // vfmm.tvv
 111100  1   vs2    vs1    001   t t t 0 1  1110111 // vfmm.alt.tvv

 111100  1   vs2    vs1    001   t t t 1 0  1110111 // vfp2mm.tvv

 111100  1   vs2    vs1    000   t t t 0 0  1110111 // vmmu.tvv
 111100  1   vs2    vs1    000   t t t 0 1  1110111 // vmms.tvv
----
...
considered part of the tail and are handled using a tail-agnostic
policy.
`vtzero.t` supports TEW values corresponding to all
supported SEW settings (8..ELEN).
This instruction is included in the Zvmbase extension.
