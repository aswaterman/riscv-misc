...
=== `Zvqldota8i` 8-bit Integer Dot-Product Extension, Version {dot-version}

The Zvqldota8i extension adds two instructions, each of which computes a dot
product between two 8-bit integer vectors, accumulating into a 32-bit scalar.
The `vqldotau.vv` instruction treats the `vs2` operand as unsigned 8-bit
integers, whereas the `vqldotas.vv` instruction treats the `vs2` operand
...
----
# Zvqldota8i 8-bit integer long dot-product instructions
# signedness of vs1 supplied by altfmt (0 unsigned, 1 signed)
vqldotau.vv vd, vs2, vs1, vm   # vd[0] += unsigned(vs2) dot vs1
vqldotas.vv vd, vs2, vs1, vm   # vd[0] += signed(vs2) dot vs1
----

These dot products are computed modulo 2^32^.

The instructions are encoded in major opcode 0x77 with funct3=0,
and with funct6=0x26 for `vqldotau.vv` and funct6=0x27 for
`vqldotas.vv`.

The Zvqldota8i extension depends on the Zve32x extension.

=== `Zvqldota16i` 16-bit Integer Dot-Product Extension, Version {dot-version}

The Zvqldota16i extension adds two instructions, each of which computes a dot
product between two 16-bit integer vectors, accumulating into a 64-bit scalar.
The `vqldotau.vv` instruction treats the `vs2` operand as unsigned 16-bit
integers, whereas the `vqldotas.vv` instruction treats the `vs2` operand
as signed 16-bit integers.
The `vtype`.`altfmt` field selects the signedness of the `vs1` operand,
with 0 indicating unsigned and 1 indicating signed.
These instructions use the same encodings as those defined in the Zvqldota8i
extension.
...
The Zvqldota16i extension depends on the Zve64x extension.
...
=== `Zvfqldota8f` OCP FP8 Dot-Product Extension, Version {dot-version}

The Zvfqldota8f extension adds two instructions, each of which computes a dot
product between two OCP FP8 vectors, accumulating into an FP32 scalar.
The `vfqldota.vv` instruction treats the `vs2` operand as E4M3 numbers,
whereas the `vfqldota.alt.vv` instruction treats the `vs2` operand
as E5M2 numbers.
...
----
# Zvfqldota8f OCP FP8 long dot-product instructions
# format of vs1 supplied by altfmt (0 E4M3, 1 E5M2)
vfqldota.vv     vd, vs2, vs1, vm   # vd[0] += E4M3(vs2) dot vs1
vfqldota.alt.vv vd, vs2, vs1, vm   # vd[0] += E5M2(vs2) dot vs1
----
...
and with funct6=0x26 for `vfqldota.vv` and funct6=0x27 for
`vfqldota.alt.vv`.

The Zvfqldota8f extension depends on the Zve32f extension.
...
=== `Zvqbdota8i` 8-bit Integer Batched Dot-Product Extension

The Zvqbdota8i extension adds instructions that perform batched dot-product
operations on 8-bit integer vectors, accumulating into 32-bit integers.
The `vqbdotau.vv` instruction treats the `vs2` operand as unsigned 8-bit
integers, whereas the `vqbdotas.vv` instruction treats the `vs2` operand
as signed 8-bit integers.
The `vtype`.`altfmt` field selects the signedness of the `vs1` operand,
with 0 indicating unsigned and 1 indicating signed.
...
----
# Zvqbdota8i 8-bit integer batched dot-product instructions
# signedness of vs1 supplied by altfmt (0 unsigned, 1 signed)
vqbdotau.vv vd, vs2, vs1, ci, vm   # vs2 unsigned
vqbdotas.vv vd, vs2, vs1, ci, vm   # vs2 signed
----

Each dot product is computed modulo 2^32^.

The instructions are encoded in major opcode 0x77 with funct3=0,
and with funct6=0x2e for `vqbdotau.vv` and funct6=0x2f for
`vqbdotas.vv`.

The Zvqbdota8i extension depends on the Zve32x extension.

=== `Zvqbdota16i` 16-bit Integer Batched Dot-Product Extension

The Zvqbdota16i extension adds instructions that perform batched dot-product
operations on 16-bit integer vectors, accumulating into 64-bit integers.
The `vqbdotau.vv` instruction treats the `vs2` operand as unsigned 16-bit
integers, whereas the `vqbdotas.vv` instruction treats the `vs2` operand
as signed 16-bit integers.
The `vtype`.`altfmt` field selects the signedness of the `vs1` operand,
with 0 indicating unsigned and 1 indicating signed.
These instructions use the same encodings as those defined in the Zvqbdota8i
extension.
...
The Zvqbdota16i extension depends on the Zve64x extension.
...
=== `Zvfqbdota8f` OCP FP8 Batched Dot-Product Extension

The Zvfqbdota8f extension adds instructions that perform batched dot-product
operations on 8-bit OCP FP8 vectors, accumulating into FP32.
The `vfqbdota.vv` instruction treats the `vs2` operand as E4M3 numbers,
whereas the `vfqbdota.alt.vv` instruction treats the `vs2` operand
as E5M2 numbers.
The `vtype`.`altfmt` field selects the datatype of the `vs1` operand,
with 0 indicating E4M3 and 1 indicating E5M2.
These instructions are reserved unless SEW=8 and LMUL=1.
...
----
# Zvfqbdota8f OCP FP8 batched dot-product instructions
# format of vs1 supplied by altfmt (0 E4M3, 1 E5M2)
vfqbdota.vv     vd, vs2, vs1, ci, vm   # vs2 E4M3
vfqbdota.alt.vv vd, vs2, vs1, ci, vm   # vs2 E5M2
----
...
The instructions are encoded in major opcode 0x77 with funct3=1,
and with funct6=0x2e for `vfqbdota.vv` and funct6=0x2f for
`vfqbdota.alt.vv`.

The Zvfqbdota8f extension depends on the Zve32f extension.
