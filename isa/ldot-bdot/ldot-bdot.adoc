...
It will not depend on a dynamic parameter such as `vl`. Dot product operations are still allowed to use `vl` or any dynamic length parameter but are encouraged to not link this to the bulk normalization parameters (which is similar to assuming identity-value products are injected in place of inactive elements).
`n` may vary depending on the element width (so have a dependence on `SEW` for example).

Note:: Given the recommendation made on `n`, RVBNA mandates a fixed value of `g` for a given format size and multiple value of `g` if `n` varies with the format size. This reduces the space of possible implementations.
Supporting multiple values of `g` is straightforward: an implementation can build a shared accumulator datapath based on the maximum `gmax` value.
Behavior for smaller `g` values would be obtained by OR-reducing both the sticky bits defined by `gmax` and the extra `gmax - g` least significant bits of the aligned products into the least significant accumulator bit. This should not require more than a few extra gates.

Note:: at the moment RVBNA is only defined for symmetric products (i.e., both factors have the same `e` and `p`). It could be later extended to asymmetric products (mixed formats) if the need arises.
...
Note:: The RVBNA pseudocode assumes that operands are IEEE encoded, but this is not actually a requirement. The pseudocode can easily be generalized to other floating-point encodings (e.g. Open Compute OFP formats).
