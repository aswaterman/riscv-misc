...
instruction fetches.
To make a store visible to instruction fetch in program order, the hart must
execute a FENCE.I instruction after the store becomes visible, as is the case
when the Ziccid extension is not implemented.
