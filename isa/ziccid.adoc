...
When a FENCE.I instruction is executed, all instruction-cache entries on this
hart corresponding to the addresses of older stores from this hart that have
not yet become globally visible are removed.

NOTE: Forcing older stores to become globally visible on a FENCE.I is
sufficient, but not necessary, to implement this property.
