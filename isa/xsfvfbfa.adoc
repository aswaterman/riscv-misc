== SiFive custom additional BF16 vector compute support *Xsfvfbfa*, Version 0.1

The Xsfvfbfa extension adds more complete BF16 vector compute support.

The Xsfvfbfa extension requires the Zve32f and Zfbfmin extensions.

NOTE: Xsfvfbfa is compatible with, but does not require, Zvfbfwma and
Zvfbfmin.

Xsfvfbfa adds a 1-bit field, `altfmt`, to the `vtype` CSR in bit position 8.
...
NOTE: SiFive implementations set `vill` in `vtype` when a reserved combination
of `altfmt` and SEW is selected.

When `altfmt`=0, the hart behaves as though Xsfvfbfa were not implemented.
...
- `vfmv.s.f` ††
- `vfmv.f.s` †
- `vfwcvt.f.f.v` (same semantics as `vfwcvtbf16.f.f.v`/`sf.vfwcvt.f.bf16.v`)
- `vfncvt.f.f.w` (same semantics as `vfncvtbf16.f.f.w`/`sf.vfncvt.bf16.f.w`)
- `vfncvt.rod.f.f.w`
- `vfrsqrt7.v`
- `vfrec7.v`
- `vfclass.v`
- `vfwmaccbf16.v[vf]` †
- `vfwcvtbf16.f.f.v`/`sf.vfwcvt.f.bf16.v` †
- `vfncvtbf16.f.f.w`/`sf.vfncvt.bf16.f.w` †
